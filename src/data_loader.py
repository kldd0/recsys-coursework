...
class DataLoader:
    ...
    def __init__(self, data_dir: str, metadata_file: str, images_dir: str, embeddings_dir: str, 
        ...
        if self.pickle_metadata_path:
            try:
                ...
                logger.info(f"Попытка загрузить pickle метаданные из: {self.pickle_metadata_path}")
                ...
                if self._pickle_adapter._df_filtered is not None and len(self._pickle_adapter._df_filtered) > 0:
                    self._use_pickle = True
                    product_count = len(self._pickle_adapter._df_filtered)
                    images_count = len(self._pickle_adapter._images) if self._pickle_adapter._images else 0
                    logger.info(f"✅ Используется pickle метаданные из {self.pickle_metadata_path}")
                    ...
                    if images_count > 0 and product_count != images_count:
                        logger.warning(f"⚠️ Количество товаров ({product_count}) не совпадает с количеством изображений ({images_count})")
                else:
                    logger.warning("⚠️ Pickle метаданные не загружены (DataFrame пуст), используется JSON")
                    logger.warning("⚠️ ВНИМАНИЕ: JSON содержит только 10 товаров, а FAISS индекс имеет 10000!")
                    self._pickle_adapter = None
                    self._use_pickle = False
                ...
                logger.warning("⚠️ ВНИМАНИЕ: JSON содержит только 10 товаров, а FAISS индекс имеет 10000!")
                logger.warning("⚠️ Результаты поиска могут быть пустыми для большинства запросов!")
                self._pickle_adapter = None
                self._use_pickle = False
        else:
            logger.warning("⚠️ Pickle файл метаданных не найден ни в одном из проверенных мест")
            logger.warning("   Проверялись пути:")
            for path in possible_paths:
                logger.warning(f"   - {path}")
            logger.warning("⚠️ Будет использован JSON файл метаданных (только 10 товаров)")
    ...
    def load_metadata(self) -> Dict:
        ...
        if self._use_pickle and self._pickle_adapter and self._pickle_adapter._df_filtered is not None:
        ...
        try:
            ...
            logger.info(f"Загружено {len(self._metadata.get('products', []))} товаров из JSON")
            return self._metadata
    ...
    def load_visual_embeddings(self) -> Optional[np.ndarray]:
        ...
        try:
            if not os.path.exists(embeddings_path):
                logger.warning(f"Файл визуальных эмбеддингов не найден: {embeddings_path}")
                return None
    ...
    def load_text_embeddings(self) -> Optional[np.ndarray]:
        ...
        try:
            if not os.path.exists(embeddings_path):
                logger.warning(f"Файл текстовых эмбеддингов не найден: {embeddings_path}")
                return None
    ...
    def get_product_image(self, product_id: str) -> Optional[Image.Image]:
        ...
        if self._use_pickle and self._pickle_adapter:
            try:
                ...
                max_index = len(self._pickle_adapter._images) - 1 if self._pickle_adapter._images else -1
                ...
                if image is None:
                    logger.debug(f"Не удалось получить изображение для индекса {index} из pickle адаптера (product_id: '{product_id}')")
                return image
            except (ValueError, TypeError) as e:
                logger.debug(f"Ошибка преобразования product_id '{product_id}' в индекс для изображения: {e}")
                # Если не удалось преобразовать, пробуем другие способы
                pass
            except Exception as e:
                logger.warning(f"Ошибка при получении изображения для product_id '{product_id}': {e}")
                return None
        ...
        if product and 'image_path' in product:
            ...
            if os.path.exists(image_path):
                try:
                    ...
                    logger.warning(f"Не удалось загрузить изображение по пути {image_path}: {e}")
    ...
    def get_product_metadata(self, product_id: str) -> Optional[Dict]:
        ...
        if self._use_pickle and self._pickle_adapter:
            try:
                ...
                # Проверяем диапазон индекса
                max_index = len(self._pickle_adapter._df_filtered) - 1 if self._pickle_adapter._df_filtered is not None else -1
                if index < 0 or (max_index >= 0 and index > max_index):
                    logger.warning(
                        f"Индекс {index} вне диапазона [0, {max_index}] для product_id '{product_id}' "
                ...
                if product_data:
                    ...
                    logger.warning(f"Не удалось получить метаданные для индекса {index} из pickle адаптера (product_id: '{product_id}')")
                    return None
            except (ValueError, TypeError) as e:
                logger.warning(f"Ошибка преобразования product_id '{product_id}' в индекс: {e}")
                # Если не удалось преобразовать, пробуем другие способы
                pass
            except Exception as e:
                logger.error(f"Неожиданная ошибка при получении метаданных для product_id '{product_id}': {e}")
                import traceback
                logger.debug(f"Traceback: {traceback.format_exc()}")
                return None
