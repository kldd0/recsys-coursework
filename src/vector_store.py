...
class FAISSVectorStore(VectorStore):
    ...
    def __init__(self, dimension: int, metric: str = "cosine"):
        ...
        logger.info(f"FAISS индекс инициализирован: dimension={dimension}, metric={metric}")
    ...
    def search(self, query_vector: np.ndarray, top_k: int) -> List[Tuple[str, float]]:
        ...
        for dist, idx in zip(distances[0], indices[0]):
            ...
            if self.metric == "cosine":
                ...
                similarity = float(1.0 / (1.0 + dist))  # Преобразование расстояния в similarity
    ...
    def load_from_file(self, path: str):
        ...
        try:
            ...
            if os.path.exists(mappings_path):
                ...
                logger.warning(f"Файл маппингов не найден: {mappings_path}. Будет использован прямой доступ по индексу.")
                self.id_to_index = {}
                self.index_to_id = {}
            
            logger.info(f"Индекс загружен из {path}: {self.index.ntotal} векторов, размерность {self.index.d}")
...
class QdrantVectorStore(VectorStore):
    ...
    def __init__(self, collection_name: str, url: str = "http://localhost:6333", 
                 api_key: Optional[str] = None, dimension: int = 512):
        """
    ...
    def search(self, query_vector: np.ndarray, top_k: int) -> List[Tuple[str, float]]:
        ...
        return [
            (hit.payload.get("product_id", str(hit.id)), hit.score)
            for hit in results
        ]
