...
class RecommendationEngine:
    ...
    def __init__(self, vector_store: VectorStore, data_loader: DataLoader, ml_interface: MLInterface):
        """
    ...
    def get_visual_recommendations(self, image: Image.Image, top_k: int = 5) -> List[Dict]:
        """
    ...
    def get_hybrid_recommendations(self, image: Image.Image, text: str, 
                                   top_k: int = 5, 
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                visual_future = executor.submit(self.get_visual_recommendations, image, top_k * 2)
                text_future = executor.submit(self.get_text_recommendations, text, top_k * 2)
                
                try:
                    visual_results = visual_future.result(timeout=config.SEARCH_TIMEOUT_SEC)
            ...
            for product in visual_results:
                ...
                product_scores[product_id]["similarity"] += product["similarity"] * visual_weight
            ...
            for product in text_results:
                ...
                if product_id not in product_scores:
                    product_scores[product_id] = product.copy()
                    product_scores[product_id]["similarity"] = 0.0
                product_scores[product_id]["similarity"] += product["similarity"] * text_weight
    ...
    def rank_results(self, results: List[Dict], weights: Optional[Dict] = None) -> List[Dict]:
        """
