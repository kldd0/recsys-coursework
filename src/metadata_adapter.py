...
class PickleMetadataAdapter:
    ...
    def _load_metadata(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Метаданные загружены из {os.path.basename(self.pickle_path)}"
            )
            ...
            if (
                ...
                or "JpegImagePlugin" in error_str
                or "__setstate__" in error_str
            ):
                logger.error("=" * 60)
                ...
                logger.error(
                    "  1. Проверьте версию Pillow: import PIL; print(PIL.__version__)"
                )
            ...
            logger.error(
                f"Доступные ключи: {list(metadata.keys()) if 'metadata' in locals() else 'N/A'}"
            )
            raise
    ...
    def get_product_image_by_index(self, index: int) -> Optional[Image.Image]:
        ...
        try:
            ...
            # Если это уже PIL Image, создаем копию для безопасности
            # Это предотвращает проблемы с внутренним состоянием PIL Image из pickle
            if isinstance(image, Image.Image):
                # Создаем копию через copy() для избежания segfault
                try:
                    return image.copy()
                except Exception as copy_error:
                    # Если copy() не работает, конвертируем через режим
                    logger.debug(
                        f"Не удалось скопировать изображение напрямую, конвертируем: {copy_error}"
                    )
                    # Конвертируем в RGB и обратно для создания нового объекта
            ...
            logger.warning(
                f"Неизвестный формат изображения для индекса {index}: {type(image)}"
            )
            return None
