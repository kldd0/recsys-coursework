...
def main():
    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            if image_processor.validate_image(uploaded_file):
                ...
                st.warning(
                    "⚠️ Изображение не прошло валидацию. Проверьте формат и размер."
                )
                uploaded_file = None

        # Кнопка поиска
        search_button = st.button(
            "🔍 Начать поиск", type="primary", use_container_width=True
        )
    ...
    if search_button:
        ...
        if not has_text and not has_image:
            st.warning(
                "⚠️ Пожалуйста, введите описание товара или загрузите изображение"
            )
            return
            ...
            try:
                ...
                if has_image and has_text:
                    ...
                    results = engine.get_hybrid_recommendations(
                        image=image, text=text_query.strip(), top_k=config.TOP_K_RESULTS
                    )
                    search_type = "hybrid"
                    ...
                    results = engine.get_visual_recommendations(
                        image=image, top_k=config.TOP_K_RESULTS
                    )
                    search_type = "image"
                    ...
                    results = engine.get_text_recommendations(
                        query=text_query.strip(), top_k=config.TOP_K_RESULTS
                    )
                    search_type = "text"
    ...
    if st.session_state.search_results is not None:
        ...
        if len(results) == 0:
            ...
            search_type_label = search_type_labels.get(
                st.session_state.last_search_type, "Поиск"
            )
