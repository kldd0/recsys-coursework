...
def generate_embeddings_for_products():
    ...
    model = ClothingEmbeddingModel(
        model_path=config.MODEL_PATH if hasattr(config, 'MODEL_PATH') and config.MODEL_PATH else "models/clothing_embedding_model.pth",
    ...
    if vectors_for_index:
        ...
        if vectors_array.shape[1] != config.VECTOR_DIMENSION:
            print(f"⚠️  Размерность эмбеддинга ({vectors_array.shape[1]}) не совпадает с конфигом ({config.VECTOR_DIMENSION})")
