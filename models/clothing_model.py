...
class CLIPEmbeddingModel:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info(
                f"CLIP модель успешно загружена. Размерность эмбеддинга: {self.embedding_dim}"
            )
    ...
    def encode_image(self, image: Image.Image) -> np.ndarray:
        ...
        """
        try:
            # Безопасная конвертация изображения (избегаем проблем с pickle)
            # Создаем новый объект через numpy array
            ...
            # Создаем новое PIL Image из массива для безопасности
            safe_image = Image.fromarray(img_array.astype(np.uint8), "RGB")

            # Обработка изображения через CLIP processor
            inputs = self.processor(
                images=[safe_image], return_tensors="pt", padding=True
            )
